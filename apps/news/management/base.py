...
class BaseImporterCommand(BaseCommand):
    option_list = BaseCommand.option_list + (
        make_option("-m", "--mezzanine-user", dest="mezzanine_user",
                    help="Mezzanine username to assign the imported news posts to."),
    ...
    def handle(self, *args, **options):
        ...
        for post_data in self.posts:
            ...
            post_data = self.trunc(NewsPost, prompt, **post_data)
            initial = {
            ...
            post, created = NewsPost.objects.get_or_create(**initial)
            ...
            for name in categories:
                cat = self.trunc(NewsCategory, prompt, title=name)
                if not cat["title"]:
                    continue
                cat, created = NewsCategory.objects.get_or_create(**cat)
                if created and verbosity >= 1:
    ...
    def add_meta(self, obj, tags, prompt, verbosity, old_url=None):
        """
        Adds tags and a redirect for the given obj, which is a news
        post or a page.
        """
        for tag in tags:
