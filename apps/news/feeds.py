...
class PostsRSS(Feed):
    ...
    def link(self):
        return self.add_domain(reverse("news_post_list"))

    def items(self):
        if not self._public:
            return []
        news_posts = NewsPost.objects.published().select_related("user").prefetch_related("categories")
        if self.tag:
    ...
    def item_author_link(self, item):
        ...
        link = reverse("news_post_list_author", kwargs={"username": username})
